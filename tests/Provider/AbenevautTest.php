...
use abenevaut\OAuth2\Provider\Abenevaut;
...
class FooAbenevautProvider extends Abenevaut
{
    protected function fetchResourceOwnerDetails(AccessToken $token)
    {
        return json_decode('{"id": 12345, "name": "mock_name", "username": "mock_username", "first_name": "mock_first_name", "last_name": "mock_last_name", "email": "mock_email", "Location": "mock_home", "link": "mock_Abenevaut_url"}', true);
    }
}

class AbenevautTest extends TestCase
{
    /**
    ...
    public function testGraphApiVersionCanBeCustomized(): void
    {
        ...
        self::assertEquals('https://api.benevaut.fr/'.$graphVersion.'/dialog/oauth', $urlAuthorize);
        self::assertEquals('https://graph.benevaut.fr/'.$graphVersion.'/oauth/access_token', $urlAccessToken);
    ...
    public function testTheBetaTierCanBeEnabled(): void
    {
        ...
        self::assertEquals('beta.api.benevaut.fr', $urlAuthorize);
        self::assertEquals('beta.graph.benevaut.fr', $urlAccessToken);
        self::assertEquals('beta.graph.benevaut.fr', $urlUserDetails);
    }
    ...
    public function testGetAccessToken(): void
    {
        ...
        self::assertNull($token->getRefreshToken(), 'Abenevaut does not support refresh tokens. Expected null.');
        self::assertNull(
            $token->getResourceOwnerId(),
            'Abenevaut does not return user ID with access token. Expected null.'
        );
    }
    ...
    {
        $this->expectException(AbenevautProviderException::class);
    ...
    {
        $provider = new FooAbenevautProvider([
            'graphApiVersion' => static::GRAPH_API_VERSION,
        ]);
